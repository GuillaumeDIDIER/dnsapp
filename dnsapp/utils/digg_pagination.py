...
# Source: http://djangosnippets.org/snippets/773/
# Author: miracle2k
# Readme: http://blog.elsdoerfer.name/2008/05/26/diggpaginator-update/
"""Implement Digg-style pagination"""
...
def _ensure_int(num, exception):
    """Raise exception if num is not an int"""
    # see Django #7307
    try:
        return int(num)
    except ValueError:
...
class ExPaginator(Paginator):
    ...
    def page(self, number, softlimit=False):
        """Implement soft limit logic"""
        try:
            return super(ExPaginator, self).page(number)
        except InvalidPage as exc:
            number = _ensure_int(number, exc)
...
class DiggPaginator(ExPaginator):
    ...
    >>> print DiggPaginator(range(1,1000), 10, body=5, padding=2, margin=2) \
    .page(3)
    1 2 3 4 5 ... 99 100
    >>> print DiggPaginator(range(1,1000), 10, body=6, padding=2, margin=2) \
    .page(4)
    1 2 3 4 5 6 ... 99 100
    >>> print DiggPaginator(range(1,1000), 10, body=5, padding=1, margin=2) \
    .page(6)
    ...
    >>> print DiggPaginator(range(1,1000), 10, body=5, align_left=True) \
    .page(1)
    1 2 3 4 5
    >>> print DiggPaginator(range(1,1000), 10, body=5, align_left=True) \
    .page(50)
    ...
    def page(self, number, *args, **kwargs):
        ...
        main_range = [int(n) for n in
            math.floor(number-body/2.0)+1,  # +1 = shift odd body to right
            math.floor(number+body/2.0)]
        # adjust bounds
        if main_range[0] < 1:
            main_range = [abs(main_range[0]-1) + n for n in main_range]
        if main_range[1] > num_pages:
            main_range = [(num_pages-main_range[1]) + n for n in main_range]
        ...
        if self.align_left:
            ...
            if main_range[1] >= num_pages-(tail+margin)+1:
                ...
                if not leading:
                    ...
                    main_range = [min(num_pages-body+1,
...
class DiggPage(Page):
    """Result of Digg pagination"""

    def __str__(self):
        return " ... ".join([
            " ".join([str(x) for x in l])
            for l in self.leading_range, self.main_range, self.trailing_range
            if l])


class QuerySetDiggPaginator(DiggPaginator, QuerySetPaginator):
    """Use Digg-style pagination with a QuerySetPaginator"""
    pass
