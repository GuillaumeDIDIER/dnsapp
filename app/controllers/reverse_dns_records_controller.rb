...
#So we can see all Reverse DNS Records.

class ReverseDnsRecordsController < ApplicationController
  ...
  def index
    ...
    @records = ReverseDnsRecord.find(:all, :conditions => hash[:conditions])
  ...
  private
    ...
    def set_conditions_and_title
      @conditions = [""]
      @title = "Tous les enregistrements Reverse DNS"
    end
